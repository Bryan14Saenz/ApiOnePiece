$(function () {
  ...
  function creadorMarine() {
    if (divMarines) {
      marines.forEach((marine) => {
        ...
        const marinePosition = document.createElement('h5');
        marinePosition.textContent = marine.posición;

        const marineApodo = document.createElement('p');
  ...
  function creadorPirata() {
    if (divPiratas) {
      piratas.forEach((pirata) => {
        ...
        const pirataPosition = document.createElement('h5');
        pirataPosition.textContent = pirata.posición;

        const pirataApodo = document.createElement('p');
        ...
        const pirataImg = document.createElement('img');
  ...
  $('#btnSearch').on('click', () => {
    ...
    const regex = new RegExp(escapeRegExp(resultado), 'i');

    function escapeRegExp(string) {
      return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    }

    const encontrado = piratas.filter((pirata) => regex.test(pirata.nombre));
    const encontradoMarines = marines.filter((marine) =>
      regex.test(marine.nombre)
    );
    ...
    if (encontradoMarines.length > 0 && divMarines) {
      encontradoMarines.forEach((marine) => {
        const card = document.createElement('div');
        card.className = 'card';
